...
3. 动态视频请求携带供应商支持的比例或尺寸参数；不支持时仍在合成前强制归一。
4. FFmpeg 对每个片段做 `scale + crop/pad + setsar + fps + format`，输出目标画布。
5. 封面取目标画布首帧，不再直接复用尺寸不确定的原始图片。
6. 字幕位置依据画布计算安全边距：竖屏底部避开平台操作栏，横屏和方屏按各自安全区处理。
...
预检失败返回明确的 4xx/5xx 错误，不创建“运行中”任务。

### 6.2 素材生成

工作流保持 Mastra 两段式结构。脚本审批后：

1. 根据 `canvasPreset` 生成统一规格分镜。
2. 每镜生成场景图，再用选定视频模型生成动态片段。
3. 任一动态片段失败，`generateScenes` 失败并终止整条 REAL 工作流，不再静态降级。
4. TTS 返回非空、可解码的语音文件和规范时间轴。
5. 配乐返回非空、可解码的音频。
6. 音频 URL 支持 `data:`、`http(s):` 和 `file:`，统一下载或解码到任务临时目录，并校验 MIME、大小和 ffprobe 音轨。

### 6.3 字幕

根据最终确认的 `script.voiceover` 生成标准 SRT：

- 时间格式为 `HH:MM:SS,mmm`；
- 每条开始时间来自脚本时间码；结束时间取下一条开始时间，最后一条以成片总时长收口；
...
不再假设供应商视频编码和尺寸一致。每个动态片段先统一为目标画布、25fps、H.264、yuv420p、方形像素；按 Scene 时长裁剪，短片段采用末帧延展。随后使用 FFmpeg filter concat 合并，避免 concat demuxer 因编码参数不一致而失败。
...
现有 `/api/video/:runId` 改为读取持久化路径并继续支持 HTTP Range。新增明确的 MP4、SRT、封面下载端点，响应使用安全文件名和 `Content-Disposition`。成片验收门只在最终校验通过后展示播放器和下载入口。
